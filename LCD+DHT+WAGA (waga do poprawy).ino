...
DHT dht1(11, DHTTYPE);  // pierwszy DHT11 → pin 11
DHT dht2(12, DHTTYPE);  // drugi DHT11   → pin 12
...
void loop() {
  ...
  // ---- 3) SAVE: jednorazowy podgląd masy ----
  ...
  if (t != thermoLast && millis() - lastThermo > DEBOUNCE) {
    ...
    if (t == LOW) {
      ...
      if (!isnan(t1)&&!isnan(h1)&&!isnan(t2)&&!isnan(h2)) {
        Serial.print("DHT1 T:"); Serial.print(t1,1);
        Serial.print("C H:"); Serial.print(h1,0);
        Serial.print("% | DHT2 T:"); Serial.print(t2,1);
